...
void UEasyGear::Simulate(FBodyInstance* bodyInst, float DeltaTime, bool apply, bool debug) {
	...
	LeftBrakeInput = FMath::Clamp(LeftBrakeInput, 0.0f, 1.0f);
	RightBrakeInput = FMath::Clamp(RightBrakeInput, 0.0f, 1.0f);

	//nose wheel
	if (NoseEnabled) {
		FVector offset = transform.GetRotation()*FVector(NoseLocationLong, 0, NoseLocationVert);
		NoseOnGround = SimulateWheel(bodyInst, DeltaTime, location + offset, downvec, fwdvec, rightvec, NoseStrutLength, NoseWheelRadius, NoseSpring, NoseShockAbsorber, NoseGrip, 0, 0,NoseGripLimit, NoseSteering, multiplier, NoseStrutCompression, NoseSkid, NoseLocation, apply, debug);
	}

	//mains
	if (MainLeftEnabled) {
		FVector offset = transform.GetRotation()*FVector(MainLocationLong, -MainLocationLat, MainLocationVert);
		MainLeftOnGround = SimulateWheel(bodyInst, DeltaTime, location + offset, downvec, fwdvec, rightvec, MainStrutLength, MainWheelRadius, MainSpring, MainShockAbsorber, MainGrip, MainBrakePower, LeftBrakeInput, MainGripLimit, false, multiplier, MainLeftStrutCompression, MainLeftSkid, MainLeftLocation, apply, debug);
	}

	if (MainRightEnabled) {
		FVector offset = transform.GetRotation()*FVector(MainLocationLong, MainLocationLat, MainLocationVert);
		MainRightOnGround = SimulateWheel(bodyInst, DeltaTime, location + offset, downvec, fwdvec, rightvec, MainStrutLength, MainWheelRadius, MainSpring, MainShockAbsorber, MainGrip, MainBrakePower, RightBrakeInput, MainGripLimit, false, multiplier, MainRightStrutCompression, MainRightSkid, MainRightLocation, apply, debug);
	}
}
...
bool UEasyGear::SimulateWheel(FBodyInstance* bodyInst, float DeltaTime, FVector loc, FVector direction, FVector fwdvec, FVector rightvec,
	float length, float radius, float spring, float shock, float grip, float brakepower, float brakeinput, float maxgrip, bool steerable, float multiplier, float &compress, bool &skid, FVector &outLocation, bool apply, bool debug) {
	compress = 0.0f;
	FHitResult result;
	...
	if (steerable) {
		...
		fwdvec = fwdvec.RotateAngleAxis(-SteeringPosition*NoseSteeringMaxAngle, direction);
		rightvec = rightvec.RotateAngleAxis(-SteeringPosition*NoseSteeringMaxAngle, direction);
	}
