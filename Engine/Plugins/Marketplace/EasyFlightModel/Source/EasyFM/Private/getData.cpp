...
void UeasyFM::GetData(EGetDataFormat Units, bool UseGravity, FVector &Velocity, FVector &TrueVelocity, float &Altitude, float &IndicatedAirSpeed, float &TrueAirSpeed, float &GroundSpeed, float &Mach, 
	...
	Altitude = (Transform.GetLocation().Z+GetWorld()->OriginLocation.Z)*AltitudeMultiplier;
...
void UeasyFM::GetAtmosphereData(FVector & Wind, float & Density, float & Pressure, float & Temperature) const
{
	UPrimitiveComponent *LocalParent = Cast<UPrimitiveComponent>(GetAttachParent());
	FBodyInstance *BodyInst = LocalParent->GetBodyInstance();
	...
	Wind = GetWind(Location);
	float AltitudeM = ((Location.Z + GetWorld()->OriginLocation.Z) / WorldScale / 100.0f);
	Density = GetAltitudeDensity(AltitudeM);
	Pressure = GetAltitudePressure(AltitudeM);
	Temperature = GetAltitudeTemperature(AltitudeM);
}
